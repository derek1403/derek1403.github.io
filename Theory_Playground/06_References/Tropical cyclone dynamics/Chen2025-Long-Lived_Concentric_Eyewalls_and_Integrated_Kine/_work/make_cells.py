# -*- coding: utf-8 -*-
"""產生 Chen2025 的 cells.jsonl 與 summary.json（paper-to-notebook 技能）。"""
import json, os

HERE = os.path.dirname(os.path.abspath(__file__))
cells = []  # 每個元素是一段 markdown 字串
...
# ===================== 標題 =====================
C(r"""# Long-Lived Concentric Eyewalls and Integrated Kinetic Energy in Tropical Cyclones (2025)
...
本研究探討**長生命期同心眼牆（long-lived concentric eyewalls, CEs）**與短生命期同心眼牆，如何伴隨不同的結構參數——例如強度 $V_{\max}$ 與**整層動能（integrated kinetic energy, IKE）**——而呈現多樣的演變。作者結合一份同心眼牆資料集，以及一套**以 AI 重建的熱帶氣旋（TC）風場剖面**，透過 IKE–$V_{\max}$ 圖檢視 CE 對 TC 演化的影響。這套 AI 風剖面融合了多頻道衛星影像與 ERA5 資料，可重建 $0\text{–}750\text{ km}$ 的軸對稱地面風剖面，其推算的 IKE 與飛機觀測相比達到 $R = 0.996$ 的高度一致。合成分析顯示：**短生命期 CE（持續 $<20\text{ h}$）傾向「維持」IKE，而長生命期 CE（占所有 CE 的 $25\%$）則有利於 IKE 進一步「成長」，造就環流範圍極為龐大的颱風。** 作者據此推論，CE 可能調控了 TC 的生命週期，使其從「小而強」演變為「大而成熟」，再到「大而弱」，最終消亡。
...
當颱風中心附近形成「兩圈」強風環時，就出現了**同心眼牆（CE）**，並會影響颱風的演變。本研究檢視 CE 如何影響 IKE（一個衡量颱風整體環流的量）。作者利用 AI 從衛星影像反演出颱風的 IKE，且與飛機觀測高度吻合（$R = 0.996$）。接著分析 CE 的影響，發現**長生命期 CE（占 $25\%$）會促進 IKE 成長，使颱風變成環流廣大的巨型風暴**。把這些變化畫在「強度–能量」圖上後，作者證明 **CE 對颱風的生命週期有實質性的影響**。""")
...
* **IKE 的提出 —— Powell & Reinhold (2007); Maclay et al. (2008); Irish (2008); Lavender & McBride (2021); Klotzbach et al. (2022)**
    ...
    * **核心概念 (Concept)：** 因此提出**整層動能 IKE**，同時結合風暴的「強度」與「大小」。
    * **方法延伸 (Extension)：** Powell & Reinhold (2007) 與 Maclay et al. (2008) 進一步提出以 $(V_{\max},\,\text{IKE})$ 的**雙參數分類系統**，因為在同一 $V_{\max}$ 下 IKE 的變異極大。
...
* **K–V 圖框架 —— Musgrave et al. (2012)**
    * **研究方法 (Approach)：** 以 **K–V 圖**摘要 TC 結構演化，把 $V_{\max}$ 與 $1{,}000\text{ km}$ 半徑內的 IKE 連結（基於 Ooyama 模式模擬）。
...
* **同心眼牆（CE）作為 K–V 圖變異來源 —— Willoughby et al. (1982); Houze et al. (2007); Rozoff et al. (2008); Terwey & Montgomery (2008); Didlake & Houze (2011); Yang et al. (2013)**
    * **理論機制 (Theoretical Mechanism)：** K–V 圖上一個重要的變異來源是**同心眼牆（CE）**——增強過程會被外眼牆的形成、內眼牆的衰減、外眼牆的收縮所打斷。
...
# ---------- 2. CE data set ----------
C(r"""### 2. Extension of the Concentric Eyewall Data Set (2007–2020)

![](./pic/Chen2025_fig1.png)
...
1. Locate the **moat** in each profile — a local $T_B$ maximum (weak convection) between two $T_B$ local minima (strong convection).
2. Verify both eyewalls satisfy $T_{B,\max} \geq \sigma_{\text{outer,min}} + T_{B,\text{outer,min}}$ and $T_{B,\max} \geq \sigma_{\text{inner,min}} + T_{B,\text{inner,min}}$, where $T_{B,\text{inner,min}}$, $T_{B,\max}$, and $T_{B,\text{outer,min}}$ are the inner-convection minimum, the moat local maximum, and the outer-eyewall minimum $T_B$; $\sigma_{\text{inner,min}}$ and $\sigma_{\text{outer,min}}$ are the corresponding standard deviations.
3. Confirm $T_{B,\text{outer,min}} \leq 230\text{ K}$ to ensure strong outer convection.
4. Require at least **five of eight sectors** to meet criteria 1–3 (symmetric CE structure).
5. Require the radial distance between the outer eyewalls of any two sectors to be $<50\text{ km}$ (to distinguish outer convection from a spiral rainband).
...
Yang et al. (2013, 2021) 用被動微波（PMW）亮溫 $T_B$ 的冰散射頻道（$85\text{–}92\text{ GHz}$）辨識 1997–2014 的 CE；本研究把西北太平洋（WNP）CE 資料集延伸到 2020，使用 SSM/I、GMI、SSM/IS、AMSR-2 等微波觀測（來自美國海軍研究實驗室 NRL）。如 **圖 1a**，PMW 亮溫能清楚捕捉到 2018 年山竹颱風的內眼牆、moat 與外眼牆。判定方法是客觀檢視 **八個 $45°$ 方位扇區**的 $T_B$ 徑向剖面（**圖 1b**）：

1. 在每個剖面找出 **moat**——兩個 $T_B$ 局部極小值（強對流）之間的一個 $T_B$ 局部極大值（弱對流）。
2. 驗證內、外眼牆都滿足 $T_{B,\max} \geq \sigma_{\text{outer,min}} + T_{B,\text{outer,min}}$ 與 $T_{B,\max} \geq \sigma_{\text{inner,min}} + T_{B,\text{inner,min}}$，其中 $T_{B,\text{inner,min}}$、$T_{B,\max}$、$T_{B,\text{outer,min}}$ 分別為內對流最小、moat 局部最大、外眼牆最小亮溫；$\sigma$ 為對應的標準差。
3. 確認 $T_{B,\text{outer,min}} \leq 230\text{ K}$，以確保外圍對流夠強。
4. 要求至少 **八個扇區中有五個**滿足條件 1–3（代表結構夠對稱）。
5. 要求任意兩扇區外眼牆的徑向距離 $<50\text{ km}$（藉此把外圍對流和螺旋雨帶區分開）。
...
# ---------- 3. DSAT IKE ----------
C(r"""### 3. Deep Learning DSAT IKE Estimates

![](./pic/Chen2025_fig2.png)
...
$$\text{IKE} = \int_V \tfrac{1}{2}\,\rho\,U^2 \, dV,$$
...
由地面風速 $U$ 在 $0\text{–}750\text{ km}$ 範圍、$1\text{ m}$ 深度內對單位體積動能積分而得，空氣密度取 $\rho = 1\text{ kg m}^{-3}$。以 $27$ 筆飛機觀測 IKE（Maclay et al. 2008）驗證後，**DSAT IKE 解釋了內核 IKE $99\%$ 的變異（$R = 0.996$、$R^2 = 0.992$）**（**圖 2b**）。密度圖（**圖 2c**）讓 $V_{\max}$ 與 IKE 能同時被監看：例如 2015 年 Patricia 颶風強度破紀錄、但 IKE **很小**；而 Katrina（2005）與 Hagibis（2019）則有**極高**的 IKE。DSAT 也能重現 $200\text{–}750\text{ km}$ 環帶的 IKE（$K_{200\text{–}750}$）與 $500\text{ km}$ 處風速（$V_{500}$）：對 $465$ 筆 ASCAT 觀測，$V_{500}$ 的 $R = 0.83$、$K_{200\text{–}750}$ 的 $R = 0.88$（**圖 2d、2e**）。**侷限：** DSAT 只用地球同步衛星影像（可能漏看 CE），且訓練資料缺乏「雙風速峰」，故眼牆置換循環期間的 IKE 演變仍待深究；但即便如此，DSAT 仍可用統計方式檢視 CE 發生前後的颱風 IKE。""")

# ---------- 4. K-V diagrams ----------
C(r"""### 4. TC Development Pathways in K–V Diagrams and CEs' Impact

![](./pic/Chen2025_fig3.png)
...
在「固定有利環境」的理想化模擬中（Musgrave et al. 2012），颱風通常會先達到**生命期最大強度（LMI）**，之後才達到生命期最大 IKE（**LME**）。但真實颱風並非如此——它們可能進入不利環境。**圖 3a** 顯示 LMI 到 LME 延遲時間的機率密度函數（PDF），並依強度分為熱帶風暴（TS，$V_{\max}\in(35,65]\text{ kt}$）、颱風（TY，$(65,96]\text{ kt}$）、強烈颱風（STY，$>96\text{ kt}$）。結果 **TS／TY 沒有顯著延遲**（它們進入不利環境後強度與 IKE 同步衰退），但 **STY 平均延遲 $20\text{ h}$**。

**圖 3b** 改依 CE 類型分組：長生命期（$n=24$）、短生命期（$n=62$）、無 CE（$n=113$）。無 CE 個案**沒有延遲**；短生命期 CE 約延遲 $\sim24\text{ h}$；長生命期 CE 延遲更顯著。**因此，CE 的形成可能是造就「大型逆時針發展路徑」的關鍵過程**——颱風藉此從「強壯的侏儒（strong dwarf）」變為「超級巨人（super giant）」，再到「虛弱的巨人（weak giant）」，最終消亡。至於強烈但無 CE 個案仍有些微延遲，則呼應 Smith et al. (2021)：颱風在巔峰強度後即使沒有眼牆置換循環，也能持續擴張——因為摩擦造成的 spin-down 抑制了增強，但外圍對流仍驅動成長；**本研究正是把這種「外圍對流」行為連結到真實颱風頻繁出現的 CE**。

合成分析（**圖 4**）同時對齊 CE 形成時間與 LMI。**$48\%$ 的強烈颱風一生至少經歷一次 CE**，而長生命期 CE（約占所有 CE 的 $26\%$）會**促進 IKE 成長**。長生命期 CE 在形成後維持強度（**圖 4a**）並促進 IKE 成長（**圖 4c、4d**）。CE 組與無 CE 組在 LMI 前的增強速率相近，但 **有 CE 的颱風在 $0\text{–}36\text{ h}$ 期間衰退較慢**，且能維持甚至增加外圍 IKE（**圖 4g、4h**）。無 CE 個案結構緊緻（$K_{750}$、$K_{200\text{–}750}$ 較小）；無 CE 組在 LMI 的 $K_{750}$/$K_{200\text{–}750}$ 約為長生命期 CE 組在形成時的 $70\%$。不過，長生命期 CE 在 $t\approx30\text{ h}$ 之後反而衰退較快，推測是因為其次生眼牆較大、有時不完整。
...
本文首先把觀測 CE 資料延伸到 2020，接著結合這份完整的 CE 資料集與 AI 反演的颱風風剖面，透過 **K–V 圖**檢視 CE 如何影響颱風的發展路徑。**短生命期 CE（持續 $<20\text{ h}$）有助維持颱風 IKE；長生命期 CE（占所有 CE 的 $25\%$）則促成顯著的 IKE 成長，造就環流極大的颱風。** 長生命期 CE 的 K–V 關係大致吻合理論，**唯獨在衰退階段例外**：理想化模擬（Musgrave et al. 2012；Ooyama 1969）永不進入不利環境，故 IKE 在 LMI 後持續成長；但真實世界中，約一半「無 CE」的強烈颱風在進入不利環境後 IKE 與 $V_{\max}$ 同步下降，另一半「有 CE」者則被 CE 調控，歷經「強壯侏儒 → 超級巨人 → 虛弱巨人」後消亡。

**作者對動力機制提出一個推測：** 既然長生命期 CE 具有更寬的 moat 與更廣的外眼牆（Yang et al. 2021），這種結構可能帶來更強的**動力效率（dynamical efficiency）**（Kuo et al. 2019），使外眼牆對流更有效地生成 IKE——這可能與「在慣性穩定度較高的區域加熱」有關，使較寬的次生眼牆比一般雨帶加熱更有效地促進 IKE 成長。後續工作將以理想化數值模擬與理論診斷深入驗證，並可擴充 CE 資料庫（更多個案／洋盆）、運用高解析重分析以改善 CE 偵測。**這篇短篇研究通訊凸顯了 AI 驅動資料在理解 CE 與颱風結構上的價值。**""")

# ---------- Data Availability ----------
C(r"""### Data Availability Statement
...
【關鍵缺口 (The Gap)】
* 理論／理想化模式很完整，但**真實觀測的 IKE 太少**，導致「長 vs 短生命期 CE 對內核／外圍 IKE 的影響」長期說不清楚。
* **本文的破口**：用 AI（DSAT）大量、均質地補上 $0\text{–}750\text{ km}$ 風剖面，讓統計分析得以成立。""")

C(r"""【引言邏輯鏈 (Logic of Introduction)】
...
C(r"""【CE 偵測法 (CE Detection)】
* **資料**：被動微波亮溫 $T_B$（冰散射頻道 $85\text{–}92\text{ GHz}$），對流越強 $T_B$ 越低。
* **判定邏輯**：沿 **八個 $45°$ 扇區**找「兩個 $T_B$ 極小（雙眼牆）夾一個 $T_B$ 極大（moat）」的結構。
* **量化門檻**：雙眼牆需滿足 $T_{B,\max}\ge \sigma_{\min}+T_{B,\min}$、外眼牆 $T_{B,\text{outer,min}}\le 230\text{ K}$、**至少 $5/8$ 扇區**達標、外眼牆徑向距離差 $<50\text{ km}$（排除螺旋雨帶）。
* **產出**：延伸到 2020 後共 **$78$ 短生命期 + $26$ 長生命期 CE**（$20\text{ h}$ 為界）；長生命期 CE 路徑偏南、偏西、移速慢。""")

C(r"""【AI 資料的「可信度橋樑」(Trust Bridge)】
* **為何重要**：作者沒有直接拿 AI 結果下結論，而是**先驗證再使用**——這是全文最關鍵的方法論態度。
...
C(r"""【核心結果：CE 怎麼改變路徑 (Core Results)】
...
C(r"""【總結與機制推測 (Summary & Conjecture)】
* **理論 vs 真實的落差**：理想化模式永不進入不利環境，IKE 在 LMI 後持續長；真實世界約一半無 CE 颱風 IKE 與 $V_{\max}$ 同步衰退，另一半靠 CE 維持與擴張——**CE 正好補上理論與真實之間的縫**。
* **動力推測（未證實）**：長生命期 CE 的寬 moat、廣外眼牆 $\Rightarrow$ 更高的**動力效率**（Kuo et al. 2019）$\Rightarrow$ 在慣性穩定區加熱，更有效地把潛熱轉成 IKE。
...
* **前提二（分析工具）**：我們接受 **K–V 圖**能有意義地摘要結構演化，且 Musgrave/Ooyama 的理想化路徑可當參考基準。
...
* **統計設計**：對齊 **CE 形成時間**與 **LMI** 兩種基準，做 long／short／no-CE 三組合成（composite）比較。""")
...
**1. 為什麼要看這一篇文章？**
因為它示範了一件當代大氣研究的關鍵能力：**當「理論很多、觀測太少」時，如何用 AI 補齊均質的大樣本資料，再回頭回答一個老問題**。它也把「CE / 眼牆置換」這個經典主題，從「強度」視角升級到「整層動能（環流大小）」視角——這正好接續了我們讀過的 [[Kuo2016 雙層模型]] 與多邊形／同心眼牆系列，是「結構動力學 → 真實統計驗證」的一塊拼圖。
...
# ===================== 輸出 =====================
out_jsonl = os.path.join(HERE, "Chen2025.cells.jsonl")
with open(out_jsonl, "w", encoding="utf-8") as f:
    for c in cells:
        f.write(json.dumps(c, ensure_ascii=False) + "\n")

summary = {
    "research_objective": "釐清長/短生命期同心眼牆(CE)是否調控熱帶氣旋在LMI之後的發展路徑(以K-V/IKE-Vmax圖呈現)，並區分長短CE對內核、外圍、總IKE的不同影響。",
    "background_premises": [
        "強度Vmax不足以描述TC的空間結構與破壞潛勢，需改用IKE(結合強度與大小)。",
        "K-V圖(Musgrave 2012)可摘要TC結構的時間演化:初生→深化→成熟。",
        "理想化軸對稱模式中IKE在進入不利環境前持續成長，真實TC則會進入不利環境而衰退。",
        "AI(DSAT)反演的軸對稱風剖面可信(IKE vs飛機R=0.996)才拿來做統計。",
    ],
    "methodology_key": [
        "延伸WNP CE資料集到2020:PMW亮溫85-92GHz、八個45度扇區判定雙眼牆+moat、門檻(>=5/8扇區, TB_outer_min<=230K, 外眼牆距離<50km)。",
        "DSAT(GAN-CNN)把多頻道衛星影像轉成0-750km軸對稱地面風剖面(1981-2020)。",
        "IKE = ∫ 1/2 ρU^2 dV (ρ=1 kg/m^3, 1m深, 0-750km)。",
        "以27筆飛機、465筆ASCAT觀測驗證IKE/V500/K200-750。",
        "合成分析:對齊CE形成時間與LMI兩基準，比較long/short/no-CE三組。",
    ],
    "key_findings": [
        "AI反演IKE vs飛機R=0.996(R^2=0.992);V500 R=0.83、K200-750 R=0.88。",
        "STY從LMI到LME平均延遲20h;no-CE無延遲、short~24h、long更長。",
        "48%強烈颱風一生至少一次CE;long-lived CE(~25-26%)促進IKE成長。",
        "CE個案在LMI後衰退較慢、外圍IKE維持或增加;no-CE結構緊緻(K750/K200-750較小,約為long-lived形成時的70%)。",
        "long-lived CE在t≈30h後強度與IKE衰退較快(次生眼牆大且常不完整)。",
    ],
    "theoretical_conclusion": "長生命期CE的K-V關係大致印證Musgrave/Ooyama的理想化增長路徑，但在衰退階段修正之(理想化不會進入不利環境)。CE補上理論恆增長與真實會衰退的落差:約一半無CE強烈颱風IKE與Vmax同步衰退，另一半有CE者被調控為強壯侏儒→超級巨人→虛弱巨人。並把Smith et al(2021)的外圍對流持續驅動增長連結到真實TC頻繁出現的CE。動力上推測長CE寬moat/廣外眼牆→更高動力效率(Kuo et al 2019)，未證實。",
    "logic_structure": "問題(CE是否調控TC發展路徑?)→方法(延伸CE資料集+AI風剖面+K-V合成)→結論(long-lived CE促進IKE成長、調控生命週期)，中間以先驗證AI資料(R=0.996)作為信任承重牆。",
    "key_terms": ["同心眼牆CE(inner eyewall/moat/outer eyewall)", "整層動能IKE", "強度Vmax/LMI/LME", "K-V圖(IKE-Vmax)", "DSAT(GAN-CNN風剖面反演)", "動力效率dynamical efficiency(Kuo et al 2019)", "眼牆置換循環ERC"],
    "key_numbers": ["R=0.996(IKE vs飛機)", "20h(long vs short CE門檻)", "0-750km風剖面;TB_outer_min<=230K", "STY平均延遲20h;short~24h", "long-lived CE約25%;48%強烈颱風至少一次CE", "no-CE在LMI的K750/K200-750約為long-lived形成時的70%"],
    "open_questions": [
        "CE增強IKE的動力機制(動力效率假說)需理想化數值模擬與理論診斷驗證。",
        "DSAT無雙風速峰、僅地球同步衛星→如何精準刻畫ERC期間IKE演變?",
        "用NumPy/Xarray自行重現:讀風剖面U(r)→徑向積分IKE→分別算K0-200/K200-750→畫K-V路徑。",
        "暖化下有利長CE的環境若改變，CE主導的巨型颱風會變多或變少?",
    ],
}
with open(os.path.join(HERE, "Chen2025.summary.json"), "w", encoding="utf-8") as f:
